...
public class Player : MonoBehaviour {
    ...
    void FixedUpdate()
    {
      ...
      if(_inputAxis.x > 0)
        {
            transform.localScale = new Vector3(_startScale, transform.localScale.y, transform.localScale.z);
        }

      if (_inputAxis.x < 0)
        {
            transform.localScale = new Vector3((-1)*_startScale, transform.localScale.y, transform.localScale.z);
        }
